    ...
    while True:
        ...
        static_flow_msgs = [
            f"Quick tape check — {t_str}. " + " ".join(
                f"{st.name} {'bullish' if (fm:=flow_metrics(st if st.cvd_ticks else PROXIES.get(st.name,st))) and fm['dir']=='BULL' else 'bearish' if fm and fm['dir']=='BEAR' else 'mixed'} at {fp(st.price, st.name)
