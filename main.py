...
GEMINI_KEY = os.getenv("GEMINI_API_KEY", "")    # free — get at aistudio.google.com, no card
GROQ_KEY   = os.getenv("GROQ_API_KEY", "")      # free — get at console.groq.com, no card
...
async def ai_voice(user_prompt: str, fallback: str = "") -> str:
    ...
    if not providers:
        log.warning("ai_voice: no API keys set — GEMINI_API_KEY and GROQ_API_KEY both empty. Using static fallback.")
        return fallback

    for p in providers:
        try:
            log.info(f"ai_voice: trying {p['name']} ({p['model']})")
            async with HTTP.post(
                ...
                if text:
                    ...
                    return text
                log.warning(f"ai_voice [{p['name']}]: empty response")
...
class SignalEngine:
    ...
    def track(self, st: CandleStore) -> list:
        ...
        if hit_sl:
            ...
            self.record["sl"] += 1
            sl_msgs = [
                f"❌ {n} stopped out at {fp(sig['sl'], n)}. Market took out our level. No stress — that's what stops are for. We protect the account and wait for the next clean setup.",
                f"❌ Stopped on {n} at {fp(sig['sl'], n)}. Structure invalidated, we're out. Take the loss clean and move on. Next setup will come.",
                f"❌ {n} hit our stop at {fp(sig['sl'], n)}. Sometimes the market takes your level before moving. We stay disciplined. {SIGNAL_COOLDOWN//3600}h cooldown then we look again.",
            ]
            return [random.choice(sl_msgs) + f"\n\n<i>{FOOT}</i>"]
        if hit_tp1 and not sig["tp1_hit"]:
            sig["tp1_hit"] = True
            self.record["tp1"] += 1
            tp1_msgs = [
                f"✅ First target hit on {n} — {fp(sig['tp1'], n)} done. Move your stop to breakeven NOW. Let the second target work. TP2 at {fp(sig['tp2'], n)}.",
                f"✅ {n} TP1 ✓ {fp(sig['tp1'], n)}. Take partials if you want. Move stop to entry. We're playing with the market's money now — TP2 at {fp(sig['tp2'], n)}.",
                f"✅ TP1 done on {n} at {fp(sig['tp1'], n)}. Secure the trade — stop to breakeven. We let the rest run to {fp(sig['tp2'], n)}.",
            ]
            return [random.choice(tp1_msgs) + f"\n\n<i>{FOOT}</i>"]
        if sig["tp1_hit"] and hit_tp2:
            del self.active[n]
            self.record["tp2"] += 1
            tp2_msgs = [
                f"🏆 Full target hit on {n}. {fp(sig['tp2'], n)} done. That's the whole move. Book it. THIS is why we wait for setups and don't chase price. Well done to everyone in the trade.",
                f"🏆 {n} TP2 ✓ {fp(sig['tp2'], n)}. Full trade complete. Beautiful setup from start to finish. Close it out and stay patient for the next one.",
                f"🏆 Both targets done on {n}. Closed at {fp(sig['tp2'], n)}. Clean trade — that's what patience and discipline look like. Account grows, we move on.",
            ]
            return [random.choice(tp2_msgs) + f"\n\n<i>{FOOT}</i>"]
        return []
...
def news_iq_pre(event: dict) -> str:
    """Pre-event analysis in human trader voice. Real data → AI generates natural language.
    Falls back to a structured static version if AI is unavailable (called sync, AI runs async separately)."""
    ...
    # Real data context for AI
    btc = next((s for s in STORES if s.name == "BITCOIN"), None)
    gold = next((s for s in STORES if s.name == "GOLD"), None)
    btc_p = fp(btc.price, "BITCOIN") if btc and btc.price else "unknown"
    gold_p = fp(gold.price, "GOLD") if gold and gold.price else "unknown"
    btc_vw = fp(btc.vwap(), "BITCOIN") if btc and btc.vwap() else "unknown"
    gold_vw = fp(gold.vwap(), "GOLD") if gold and gold.vwap() else "unknown"
    c = CTX.get("BITCOIN", {})
    funding_note = "longs are crowded" if c.get("funding", 0) > 0.02 else "shorts are crowded" if c.get("funding", 0) < -0.02 else "positioning is neutral"

    # SMC levels for context
    btc_smc = SMC_ENGINE.analyse(btc) if btc else {}
    gold_smc = SMC_ENGINE.analyse(gold) if gold else {}
    btc_liq = btc_smc.get("liq", [])
    gold_liq = gold_smc.get("liq", [])

    level_context = []
    if btc_liq:
        level_context.append(f"BTC liquidity at {fp(btc_liq[0]['level'], 'BITCOIN')} ({btc_liq[0]['type']})")
    if gold_liq:
        level_context.append(f"Gold liquidity at {fp(gold_liq[0]['level'], 'GOLD')} ({gold_liq[0]['type']})")

    # Store the AI prompt — the actual AI call happens async in tick_worker
    # This function returns a static version as the immediate fallback
    static = (
        f"{emoji} <b>{title.upper()} dropping at {dt_str}</b>\n\n"
        f"Last reading was {previous}, market expecting {forecast}.\n\n"
        f"If it beats → {beat_rx}\n"
        f"If it misses → {miss_rx}\n\n"
        f"{note}\n\n"
        f"BTC at {btc_p} (VWAP {btc_vw}) · Gold at {gold_p} (VWAP {gold_vw}) · {funding_note}\n\n"
        + (f"Levels to watch: {', '.join(level_context)}\n\n" if level_context else "")
        + ("🔴 Signals paused until the number prints." if impact == "High" else "🟡 Watch for a spike.")
        + f"\n\n<i>{BRAND}</i>"
    )
    return static

async def news_iq_pre_ai(event: dict) -> str:
    """Async AI version of news_iq_pre — generates truly human trading commentary.
    Called from tick_worker when an event is first detected."""
    title    = event.get("title", "?")
    country  = event.get("country", "")
    impact   = event.get("impact", "")
    forecast = event.get("forecast", "")
    ...
    gold_vw = fp(gold.vwap(), "GOLD") if gold and gold.vwap() else "unknown"
    c = CTX.get("BITCOIN", {})
    fr_note = "longs are crowded" if c.get("funding", 0) > 0.02 else "shorts are crowded" if c.get("funding", 0) < -0.02 else "market is not crowded either way"
    btc_smc = SMC_ENGINE.analyse(btc) if btc else {}
    gold_smc = SMC_ENGINE.analyse(gold) if gold else {}
    liq_b = btc_smc.get("liq", []); liq_g = gold_smc.get("liq", [])
    prompt = f"""News event coming up:
Event: {title} ({country}, {impact} impact)
Time: {dt_str}
...
If miss: {miss_rx}
Context: {note}
Market right now: BTC at {btc_p} vs VWAP {btc_vw}, Gold at {gold_p} vs VWAP {gold_vw}, {fr_note}
...
HELP_TEXT = (
    ...
    "/dayoutlook — today's bias\n"
    "/sotd — signal of the day (BTC + Gold)\n"
    "/signal — open signals + record\n"
    "/weekend — weekly review\n"
    "/health — feed status\n\n"
    "Auto: 07:00 outlook · 07:10 news calendar · session opens · hourly flow\n"
    "     · alerts · signals · 10:00 Sat weekly review · 21:30 Sun reopen\n\n"
    "Signals: BTC + Gold · ≥10/12 · max 2/day · 4h cooldown\n"
    "NewsIQ fires 15min before any High/Medium release + aftermath 10min after\n\n"
        ...
        try:
            ...
            for title, dt, impact in news_imminent(15):
                ...
                if ev_key not in NEWS_IQ_SNAPSHOTS:
                    ...
                    iq = await news_iq_pre_ai(ev_obj)   # async AI version
                    if iq:
                        msgs.append(iq)
            ...
            for st in stores:
                ...
                if sig:
                    # Generate AI human trade call using the real levels from the signal
                    active = SIGNAL_ENGINE.active.get(st.name, {})
                    if active and (GEMINI_KEY or GROQ_KEY):
                        side = "LONG" if active["dir"] == "BULL" else "SHORT"
                        ai_prompt = (
                            f"Trade fired: {st.name} {side}\n"
                            f"Entry {fp(active['entry'], st.name)}, "
                            f"Stop loss {fp(active['sl'], st.name)}, "
                            f"TP1 {fp(active['tp1'], st.name)}, "
                            f"TP2 {fp(active['tp2'], st.name)}\n"
                            f"Write the trade call for the group. Include ALL the exact numbers. "
                            f"Sound like a trader calling the trade live. Max 3-4 sentences. "
                    ...
                    if png:
                        photos.append((png, final_sig))
                    else:
                        msgs.append(final_sig)
            for m in msgs:
                await tg_send(m)
...
async def flow_update_worker(stores):
    while True:
        ...
        for st in stores:
            ...
            c = CTX.get(st.name, {})
            vw = st.vwap()
            bid_w = c.get("bid_wall"); ask_w = c.get("ask_wall")
            _, acc = cvd_acceleration(fs)
...
async def session_worker(stores, proxies, h4, ctx):
    ...
    while True:
        ...
        if s and t.minute < 2 and (t.strftime("%Y%m%d") + s) not in opened:
            ...
            SESSION_OPEN = {
                "ASIA":     ["Asian session just opened. Volume is thin early — price can be deceptive here, traps are common. Don't over-trade.",
                             "Asia is live. Low volume session — what happens here often reverses in London. Stay patient."],
                "LONDON":   ["London just stepped in. This is where the real volume starts — take the tape seriously from here.",
                             "London open. Liquidity picks up fast. Watch for the initial sweep before the real move."],
                "NEW YORK": ["New York is here. Highest volume session of the day — if there's a move to be made, this is where it happens. Stay focused.",
                             "NY just opened. Peak liquidity, biggest moves. We want clean setups only — don't force anything."],
                "NY PM":    ["Afternoon session. London is wrapping up — volume drops off from here. Better to manage open trades than look for new ones.",
                             "NY afternoon. Thinner liquidity, choppier price action. If you're already in a trade, manage it. If not, be selective."],
            }
            # Build human static fallback
            asset_lines = []
            for st in stores:
                if st.name == "GOLD" and not gold_market_open():
                    continue
                ...
                vw = st.vwap()
                p = st.price
                dir_word = DIR_WORD[fm["dir"]].lower() if fm and fm["dir"] != "NEUTRAL" else "mixed"
                vwap_note = f" — VWAP at {fp(vw, st.name)}" if vw else ""
                asset_lines.append(f"{st.name} at {fp(p, st.name) if p else '—'}, tape is {dir_word}{vwap_note}.")
...
async def close_worker(stores):
    ...
    while True:
        ...
        if t.hour == 21 and 0 <= t.minute < 2 and sent_for != t.date():
            ...
            close_fallbacks = [
                f"🌙 That's the NY session done for {t.strftime('%A')}. " + " · ".join(f"{st.name} closed at {fp(st.price, st.name)}" for st in stores if st.price) + ". Good trading everyone — reset, rest, see you tomorrow.",
                f"🌙 NY books are closed. " + " · ".join(f"{st.name} {fp(st.price, st.name)}" for st in stores if st.price) + ". Review your trades, no emotion. Fresh start tomorrow.",
            ]
